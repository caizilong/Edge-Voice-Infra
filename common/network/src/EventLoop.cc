...
void EventLoop::resumeInLoop(std::coroutine_handle<> handle) {
  if (!handle) {
    return;
  }
  if (isInLoopThread()) {
  ...
  queueInLoop([handle]() mutable { handle.resume(); });
}
...
void EventLoop::abortNotInLoopThread() {
  LOG(FATAL) << "EventLoop thread mismatch, expected " << threadId_
             << ", but current thread is " << getThreadId();
}
