...
StackFlow::StackFlow(const std::string& unit_name)
    ...
    event_queue_.appendListener(LOCAL_EVENT::EVENT_PAUSE,
                                std::bind(&StackFlow::_pause, this, std::placeholders::_1));
    event_queue_.appendListener(LOCAL_EVENT::EVENT_EXIT,
                                std::bind(&StackFlow::_exit, this, std::placeholders::_1));
    event_queue_.appendListener(LOCAL_EVENT::EVENT_SETUP,
                                std::bind(&StackFlow::_setup, this, std::placeholders::_1));
    event_queue_.appendListener(LOCAL_EVENT::EVENT_TASKINFO,
                                std::bind(&StackFlow::_taskinfo, this, std::placeholders::_1));
    rpc_ctx_->register_rpc_action("setup", std::bind(&StackFlow::_rpc_setup, this,
                                                     std::placeholders::_1, std::placeholders::_2));
    rpc_ctx_->register_rpc_action("pause", std::bind(&StackFlow::_rpc_pause, this,
                                                     std::placeholders::_1, std::placeholders::_2));
    rpc_ctx_->register_rpc_action(
            "exit",
            std::bind(&StackFlow::_rpc_exit, this, std::placeholders::_1, std::placeholders::_2));
    rpc_ctx_->register_rpc_action("taskinfo",
                                  std::bind(&StackFlow::_rpc_taskinfo, this, std::placeholders::_1,
                                            std::placeholders::_2));
...
int StackFlow::setup(const std::string& work_id, const std::string& object,
...
int StackFlow::exit(const std::string& work_id, const std::string& object,
                    const std::string& data) {
...
void StackFlow::pause(const std::string& work_id, const std::string& object,
                      const std::string& data) {
...
void StackFlow::taskinfo(const std::string& work_id, const std::string& object,
                         const std::string& data) {
    nlohmann::json error_body;
    error_body["code"] = -18;
    error_body["message"] = "not have unit action!";
...
int StackFlow::sys_register_unit(const std::string& unit_name) {
    ...
    unit_call("sys", "register_unit", unit_name,
              [&](const std::shared_ptr<StackFlows::ZmqMessage>& pzmg_msg) {
...
void StackFlow::sys_sql_unset(const std::string& key) { unit_call("sys", "sql_unset", key); }